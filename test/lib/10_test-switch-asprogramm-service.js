...
describe('Homematic Plugin (index)', function () {
  let data = '{"devices":[]}'
  let that = this
  var config = { ccu_ip: '127.0.0.1', subsection: 'HomeKit', testdata: data, ios10: true, programs: ['TestProgram'] }
  var platform = new homebridgeMock.PlatformType(log, config, homebridgeMock)

  before(function () {
    log.debug('Init Platform Switch(Program mode) Service Test)')
  ...
  describe('Homebridge Platform Switch(Program mode) Service Test', function () {
    it('test accessory build', function (done) {
      ...
      let s = ac.getService(Service.Outlet)
