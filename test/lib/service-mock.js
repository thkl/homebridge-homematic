...
Service.prototype.addCharacteristic = function (characteristic) {
  ...
  for (var index in this.characteristics) {
    ...
    if (existing.UUID === characteristic.UUID) { throw new Error('Cannot add a Characteristic with the same UUID as another Characteristic in this Service: ' + existing.UUID) }
  }
...
Service.prototype.addOptionalCharacteristic = function (TheCharacteristic) {
  // characteristic might be a constructor like `Characteristic.Brightness` instead of an instance
  // of Characteristic. Coerce if necessary.
  if (typeof TheCharacteristic === 'function') { TheCharacteristic = new TheCharacteristic() }

  this.optionalCharacteristics.push(TheCharacteristic)
}
